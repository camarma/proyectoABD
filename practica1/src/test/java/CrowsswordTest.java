...
public class CrowsswordTest {
	...
	public void changePassword() {
		...
		String pw = dao.getPassword("user");
		assertEquals("Contrasena cambiada de 'user'", "krocsyldiphic", pw);
		
		dao.modifyPassword("user", "pass");
		String pw2 = dao.getPassword("user");
		assertEquals("Contrasena restaurada de 'user'", "pass", pw2);
	}
	...
	public void crucigramasDeUser() {
		...
		assertTrue("Tras anadir 'Crucigrama2' user debe tener al menos un crucigrama asociado", cw.size() >= 1);
		...
		assertTrue("Tras anadir 'Crucigrama2' a 'user', éste debe estar en su lista de crucigramas", foundCW2);
	}
